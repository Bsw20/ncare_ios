        ...
        ZStack {
            ...
            NavigationView{
                ...
                VStack(alignment: .leading){
                    Text("Briefly explain what happened \nnear you").padding(.leading, 22).opacity(0.7).padding(.top,5).padding(.bottom, 15)
                    ...
                    Button(action: { // BUTTON TO CHANGE FORMS OF AUTHORISATIONS
                                ...
                                if defaults.object(forKey: "longitude") != nil
                                {
                                    ...
                                    self.socketData.emit("reports:create", with:  [ "name" : self.title, "description" : self.description , "type": self.report.type, "coordinates": [defaults.double(forKey: "longitude"),defaults.double(forKey: "latitude")] ] )
                                }
                            // emit
                        }) {
                  ...
                  Text(showerror ? "Some error appeared": "").foregroundColor(.red).animation(.spring())
               ...
               .modifier(DismissingKeyboard())
                ...
                if(downmenuint == 2)
                {
                    Feed(posts2: []).navigationBarTitle("Reports")
                }
                else
                {
                    ProfileView().navigationBarTitle("Your profile")
                }
        }
            VStack {
                VStack {
                    Spacer()
                    DownMenu(downMenuButton: self.$downmenuint).padding(.top, -130).overlay(
                        RoundedRectangle(cornerRadius: 5)
                            .stroke(Color.primary . opacity(0.1), lineWidth: 1.5)
...
class Report: ObservableObject {
    ...
    static let types = ["Killing of an animal", "Setting fire", "Unauthorized fishing", "Hunting", "Unauthorized gathering","Destruction of nature", "Other"]
    ...
    var body: some View {
        ...
        VStack {
            ...
            Text("")) {
                ...
                }.padding(.trailing,30).foregroundColor(maincolor)
            .navigationBarTitle("Choose type of report")
            
            Text("Any human activity brings along with it, many side effects. One aspect of it could bring about development while the other side of the coin could be a detrimental effect. These unfavourable changes often lead to environmental issues that affect the natural balance of the environment. Some of the major environmental issues that are causing immense concern are environmental pollution, air pollution, water pollution, garbage pollution, noise pollution, deforestation, resource depletion, hunting etc.").opacity(0.7).padding(.leading, 22).padding(.trailing, 22).padding(.top, 10)
            Spacer()
        }
